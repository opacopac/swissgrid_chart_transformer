...
impl fmt::Display for MapTileCoord {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "x: {}, y: {}, z: {}", self.x_tile, self.y_tile, self.zoom)
    }
}
...
impl MapTileCoord {
    pub fn from_position(pos: Position2d, zoom: u32) -> MapTileCoord {
        ...
        return MapTileCoord { x_tile, y_tile, zoom };
    }


    pub fn to_position(&self) -> Position2d {
        let n = 2_u32.pow(self.zoom) as f32;
        let lon = self.x_tile as f32 / n * 360.0 - 180.0;
        let lat = (((PI * (1.0 - 2.0 * self.y_tile as f32 / n)).sinh()).atan()).to_degrees();
